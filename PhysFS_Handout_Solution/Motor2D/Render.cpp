...
// Blit to screen
bool ModuleRender::Blit(SDL_Texture* texture, int x, int y, float rescale, const SDL_Rect* section, bool fliped, float pivotX, float pivotY, float speedX, float speedY, double angle, int rotpivot_x, int rotpivot_y) const
{
	bool ret = true;
	uint scale = app->win->GetScale();

	SDL_Rect rect;
	rect.x = (int)(camera.x * speedX) + x * scale * rescale;
	rect.y = (int)(camera.y * speedY) + y * scale * rescale;
	...
	rect.w *= scale * rescale;
	rect.h *= scale * rescale;
